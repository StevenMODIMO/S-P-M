...
export default function MarkdownEditor() {
  ...
  return (
    <main className="h-[95vh] bg-black">
      <header className="flex gap-1 p-1">
        <div
          className={`${inter.className} text-white text-xl`}
          onClick={() => setTab(0)}
        >
          Edit
        </div>
      ...
      {tab === 0 ? (
        <textarea
          className="w-full h-[95%] outline-none p-2 text-white bg-[#1a1a1a]"
          value={markdown}
          onChange={(e) => setMarkdown(e.target.value)}
        ></textarea>
      ) : (
        <div className="bg-black text-white h-[95%] w-full overflow-auto p-3">
          <Markdown
